...
function Rocket(parentElement){
  ...
  this.create = function(){
    this.rocket.style.height = this.bulletHeight+'px';
    this.rocket.style.width = this.bulletwidth+'px';
    this.rocket.style.position = 'absolute';
    this.rocket.style.top = this.y+'px';
    this.rocket.style.left = this.x+'px';
    this.mainElement.appendChild(this.rocket);
    this.rocket.style.zIndex = 20;
    var rocketImage = document.createElement('img');
    this.rocket.appendChild(rocketImage);
  ...
  this.update = function(){
    this.y = -10;
    this.rocket.style.top = y+'px';
  }

  this.deleteRocket = function(){
    this.mainElement.removeChild(rocket);
  }
}
    ...
    backgroundMove = setInterval(function() {
      ...
      document.onkeydown = function(event) {
        ...
        if (key===37||key===65){
          ...
          if(that.rocket.rocketNumber!=0){
            that.rocketCreate();
            that.rocket.rocketNumber--;
            console.log(that.rocket.rocketNumber);
          }
        }
      };
